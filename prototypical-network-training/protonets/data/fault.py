...
def load_class_images(d):
    ...
    if d['class'] not in DATA3D_CACHE:
        class_indices = [i for i, (_, label) in enumerate(d['dataset']) 
                       if str(label) == d['class']]
        
        if len(class_indices) == 0:
            raise Exception(f"No images found for class {d['class']}")
        ...
        DATA3D_CACHE[d['class']] = torch.stack(class_data)
    
    return {'class': d['class'], 'data': DATA3D_CACHE[d['class']]}
...
def load(opt):
    ...
    for split, setting in settings.items():
        ...
        transforms_list = [
            lambda x: {'class': x, 'dataset': dataset, 'transform': setting['transform']},
            load_class_images,
