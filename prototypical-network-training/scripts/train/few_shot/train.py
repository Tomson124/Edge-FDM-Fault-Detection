...
def main(opt):
    ...
    def on_end_epoch(hook_state, state):
        ...
        if val_loader is not None:
            if opt['train.save_all']:
                os.makedirs(os.path.join(opt['log.exp_dir'], 'checkpoints'), exist_ok=True)
                torch.save(state['model'], os.path.join(opt['log.exp_dir'], 'checkpoints/{:d}.pt'.format(state['epoch'])))
